<?php
require_once(base_path()."/vendor/Wxpay/lib/WxPay.Exception.php");
require_once(base_path()."/vendor/Wxpay/lib/WxPay.Config.php");
require_once(base_path()."/vendor/Wxpay/lib/WxPay.Data.php");
...
class WxPayApi
{
     ...
     * @return 成功时返回，其他抛异常
     */
    public static function unifiedOrdercx($inputObj, $timeOut = 6)
    {
        $url = "https://api.mch.weixin.qq.com/pay/unifiedorder";
        //检测必填参数
        if(!$inputObj->IsOut_trade_noSet()) {
        ...
        $inputObj->SetAppid(WxPayConfig::CXAPPID);//公众账号ID
        $inputObj->SetMch_id(WxPayConfig::MCHID);//商户号
//        WxPayConfig::KEY=   // 重写 key 与
        $inputObj->SetSpbill_create_ip($_SERVER['REMOTE_ADDR']);//终端ip
        //$inputObj->SetSpbill_create_ip("1.1.1.1");
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串
