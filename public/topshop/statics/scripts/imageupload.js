var imageUpload = function(options) {
    ...
    .on('click','.action-save',function(){
        if(isMultiple && isMultiple == 'true') {
            var multipleList = '';
            var display_bool = $("#display_row").val()?"1":"0";
            var display_bool_1 = $("#display_row_1").val()?"1":"0";
            var display_exploration = $("#display_exploration").val()?"1":"0";
            //获取当前时间
            var d=new Date()
            var day=d.getDate()
            ...
            if(display_bool == "1"||display_exploration == "1"||display_bool_1 == "1"){
            var new_img = [];
            }
            ...
            $('#gallery_modal').find('.thumbnail.checked').each(function(){
                ...
                if(display_bool == "1"||display_exploration == "1"||display_bool_1 == "1"){
                item.img_name = $(this).find('.image-name').html();
                }
            ...
            if(display_exploration == "1"){
                for (var i = 0; i < new_img.length; i++) {
                    ...
                    + '<input type="hidden" name="'+ name +'" value="'+ new_img[i].img_id +'">'
                    + '<div class="img-put">'
                    + '<img src="'+ new_img[i].img_src +'">'
                    + '<i class="glyphicon glyphicon-picture"></i>'
                    + '</div>'
                    + '</a><input type="hidden" name="image_name[]" value="'+new_img[i].img_name+'">'
                    + '</div>';
                }
            }else if(display_bool == "1"){
                    for (var i = 0; i < new_img.length; i++) {
                        multipleList +=  '<div class="multiple-item">'
                        + '<div class="multiple-del glyphicon glyphicon-remove-circle"></div>'
                        + '<a class="select-image">'
                        ...
                        + '<i class="glyphicon glyphicon-picture"></i>'
                        + '</div>'
                        + '</a><div class="form-control">图片名称：'+new_img[i].img_name+'<input type="hidden" name="image_name[]" value="'+new_img[i].img_name+'"></div><div><input type="txt" name="description[]" required class="form-control" placeholder="设计说明" /></div>'
                        + '</div>';
                    }
            }else if(display_bool_1 == "1"){
                for (var i = 0; i < new_img.length; i++) {
                    multipleList +=  '<div class="multiple-item">'
                        + '<div class="multiple-del glyphicon glyphicon-remove-circle"></div>'
                        + '<a class="select-image">'
                        ...
                        + '<i class="glyphicon glyphicon-picture"></i>'
                        + '</div>'
                        + '</a><div class="form-control">图片名称：'+new_img[i].img_name+'<input type="hidden" name="image_name[]" value="'+new_img[i].img_name+'"></div><div class="form-control">图纸编号：SMJ'+year+month+day+hours+min+sec+i+'<input type="hidden" name="number[]" value="SMJ'+year+month+day+hours+min+sec+i+'"></div><div class="form-control">花费时间：<input type="number" name="use_time[]" min="0.1" maxlength="6" style="width: 30%" value="0">(小时)</div>'
                        + '</div>';
                }
            }else{
