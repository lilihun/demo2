...
'<img src="'+a.url+'"><input type="hidden" name="'+v+'" value="'+a.image_id+'">'),$(s).html(f)}else a.message&&($("#messagebox").message(a.message),e.val(""))},error:function(){$("#messagebox").message("上传出错，请重试")}})});$("#gallery_modal").on("shown.bs.modal",function(a){h=[];c=$(a.relatedTarget);e=c.attr("data-limit");g=c.attr("data-isMultiple");d=c.attr("data-name");c.parents(".multiple-upload").find(".multiple-item");f=c.parent().find(".multiple-item");$(this).attr("data-ids",h);f&&($(f).each(function(a,
b){var e={};e.img_id=$(b).find('input[name^="list"]').val();e.img_src=$(b).find("img").attr("src");h.push(e)}),b())}).on("click",".gallery-modal-tabs li a",function(a){a.preventDefault();var e=$(this);$(this).parent().addClass("active").siblings(".active").removeClass("active");a=$(this).attr("href");$.post(a,function(a){e.parents(".nav-tabs-custom").find(".gallery-modal-content").empty().append(a);b()})}).on("click",".action-save",function(){if(g&&"true"==g){var a="",b=h;$("#gallery_modal").find(".thumbnail.checked").each(function(){var a=
