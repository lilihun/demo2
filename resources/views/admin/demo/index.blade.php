...
@section('content')
    ...
    <a href="{{url('admin/demo/create')}}" class="btn btn-primary margin-bottom"><i class="fa fa-paint-brush" style="margin-right: 6px"></i>创建</a>
    <div class="box box-primary">
        <div class="col-xs-14 col-sm-14">
            <div class="nav-tabs-custom" id="tabs">
                <ul class="nav nav-tabs sub-nav-tabs">
                    <li role="presentation" class="active"><a href="{{ url("admin/demo/index")}}">中文版</a></li>
                    ...
                    <table class="table table-hover table-bordered">
                        ...
                        @if(count($res)>0)
                            @foreach($res as $v)
                                <tr>
                                    <td>
                                        <a style="font-size: 16px" href="{{ url("/admin/demo/edit/$v->news_id") }}"><i class="fa fa-fw fa-pencil" title="修改"></i></a>
                                        <a style="font-size: 16px;color: #dd4b39;" href="javascript:;" onclick="delCate({{$v->news_id}})"><i class="fa fa-fw fa-trash-o" title="删除"></i></a>
                                    </td>
                                    <td class="text-muted">{{ $v->title }}</td>
                                    <td class="text-navy">{{ $v->order }}</td>
                                    <td class="text-red">@if($v->disabled=='0') 显示 @else 不显示 @endif</td>
                                    <td class="text-navy">{{ date("Y-m-d H:i:s",$v->create_time) }}</td>
